...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	{
			scoreTxt = new FlxText(0, FlxG.height * 0.89 + 60, FlxG.width, "YOU DIED!", 60);
			scoreTxt.setFormat(Paths.font("din.otf"), 60, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		FlxG.camera.zoom = FlxMath.lerp(0.75, FlxG.camera.zoom, 0.95);
		...
		if(PlayState.deathByHP)
			{
				scoreTxt.text = "Whoop! You died! Oh well.";
			}
		else if(PlayState.deathByR)
			{
				scoreTxt.text = "Avoid pressing your Reset key next time.";
			}

		if (controls.ACCEPT)
		{
			endBullshit();
			PlayState.deathByHP = false;
			PlayState.deathByR = false;
		}
