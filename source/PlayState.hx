...
class PlayState extends MusicBeatState
{
	...
	public var timeStuff:FlxTypedGroup<FlxObject>;
	...
	public var ferStart:BGSprite;
	public var ferHall:FlxSprite;
	public var ferPad:BGSprite;
	public var ferJail:BGSprite;
	public var padBF:Boyfriend;
	public var padOpp:Character;
	public var padZoom:Float = 0;

	public static var deathByHP:Bool;
	public static var deathByR:Bool;
	...
	override public function create()
	{
		...
		defaultCamZoom = stageData.defaultZoom + padZoom;
		...
		switch (curStage)
		{
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				ferJail = new BGSprite('fero/jailCell', -600, -200, 0.9, 0.9);
				add(ferJail);

				ferHall = new FlxSprite(-600 , -200);
				ferHall.frames = Paths.getSparrowAtlas('fero/runningThroughTheHalls');
				ferHall.antialiasing = ClientPrefs.globalAntialiasing;
				ferHall.animation.addByPrefix('scroll', 'Scroll ', 24);
				add(ferHall);

				ferPad = new BGSprite('fero/padWIP', 175, 100, 0, 0);
				add(ferPad);

				padBF = new Boyfriend(-750, 500, "bfPAD");
				padBF.dance();
				add(padBF);
				//padOpp = new Character(-)

				
				ferJail.setGraphicSize(Std.int(ferJail.width * 1.1));
				ferHall.setGraphicSize(Std.int(ferHall.width * 1.1));
				ferPad.setGraphicSize(Std.int(ferPad.width * 1.7));

				gfGroup.visible = false;

				ferStart = new BGSprite('fero/schoolBG', -600, -200, 0.9, 0.9);
				add(ferStart);
				ferStart.setGraphicSize(Std.int(ferStart.width * 1.1));
		}
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 95, 400, "", 32);
		timeTxt.setFormat(Paths.font("din.otf"), 32, FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 2;
		timeTxt.visible = !ClientPrefs.hideTime;
		if(ClientPrefs.downScroll) timeTxt.y = STRUM_X + (FlxG.width / 2) - 65;
		if(ClientPrefs.middleScroll) timeTxt.x = STRUM_X + 750;
		...
		FlxG.camera.follow(camFollowPos, LOCKON, lerpBruk);
		...
		/*healthText = new FlxText(0, 0, healthBarBG.width + 100, "50%", 20);
		healthText.x = healthBar.x + 100;
		//add(healthText);
		...
		textBG = new FlxSprite(0, FlxG.height * 0.89 + 34).makeGraphic(FlxG.width, 30, FlxColor.BLACK);
		//textBG.y = scoreTxt.y;
		textBG.alpha = 0.35;
		textBG.cameras = [camOther];
		add(textBG);

		var stagedFont:String;
		var stagedFontSize:Int;

		if(stageData.isPixelStage) 
			{
				stagedFont = "pixel.otf";
				stagedFontSize = 15;
			}
		else
			{
		...
		scoreTxt = new FlxText(0, FlxG.height * 0.89 + 37, FlxG.width, "Score: 0 | Misses: 0 | Accuracy: 0%", stagedFontSize);
		scoreTxt.setFormat(Paths.font(stagedFont), stagedFontSize, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var kadeEngineWatermark = new FlxText(0, FlxG.height * 0.89 - 630, 0, SONG.song + " - AFNFVSM Engine (Psych 0.4.2)", 16);
			kadeEngineWatermark.setFormat(Paths.font("din.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			kadeEngineWatermark.scrollFactor.set();
			kadeEngineWatermark.screenCenter(X);
			kadeEngineWatermark.borderSize = 1.25;
			kadeEngineWatermark.alpha = 0.5;
			kadeEngineWatermark.antialiasing = ClientPrefs.globalAntialiasing;
			add(kadeEngineWatermark);
			//copied from kade engine because its bad
			kadeEngineWatermark.cameras = [camOther];

		var scanlines:FlxSprite = new FlxSprite(0, 0, Paths.image('weeb/scanlines', 'week6'));
		scanlines.screenCenter(XY);
		scanlines.alpha = 0.35;
	...
	var padObj:FlxObject;
	var offsetX:Float;
	var offsetY:Float;

	public function sickFeralSwitch(name:String, shouldFlash:Bool):Void
	{
		switch(name)
		{
			case 'enter':
				ferStart.visible = true;
				ferHall.visible = false;
				ferJail.visible = false;
				ferPad.visible = false;
				...
				FlxG.camera.zoom == 1;
			case 'jail':
				ferStart.visible = false;
				ferHall.visible = false;
				ferJail.visible = true;
				ferPad.visible = false;
				for (i in 0...opponentStrums.length) {
					opponentStrums.members[i].visible = true;
				}
			...
			case 'pad':
				...
				padBF.animation.play("idle");
				padZoom -= 0.5;
				FlxG.camera.follow(padBF, LOCKON, 5);

		}

		if(shouldFlash && ClientPrefs.flashing)
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) {
				...
				if(ClientPrefs.middleScroll || SONG.song == "Letter A Song" || SONG.song == "Letter S Song" /*for testing*/ || SONG.song == "Test") opponentStrums.members[i].visible = false;
			}
	...
	function dodgeStuff():Void //thanks hat i love you!!!!!! platonically!!!!!
		{
			if (SONG.song.toLowerCase() == 'blammed'){
				warning = true;
				FlxG.sound.play(Paths.sound('confirmMenu'), .7);
				serenityLights.alpha = .5;
				FlxTween.tween(serenityLights, {alpha: 0}, 1, {ease: FlxEase.cubeInOut});
				new FlxTimer().start(0.5, function(timeThingh:FlxTimer)
				{
					warning = false;
					if(boyfriend.animation.curAnim.name == 'dodge' || cpuControlled){
						var dodgechar:Character = dad;
						var dodgedadachar:Character = dad;
						dodgechar = boyfriend;
						if (cpuControlled){
							dodgechar.playAnim('dodge', true);
							dodgetimer = true;
						...
						dodgedadachar = dad;
						dodgedadachar.playAnim('gun', true);
						dodgedadachar.specialAnim = true;
						dodgechar.specialAnim = true;
						FlxG.sound.play(Paths.sound('gun'), 0.5);
						FlxG.camera.shake(0.005, .25);
					} else {
						FlxG.sound.play(Paths.sound('damagesfx'), 0.5);
						var hitchar:Character = dad;
						var dodgedadachar:Character = dad;
						hitchar = boyfriend;
						hitchar.playAnim('hurt', true);
						hitchar.specialAnim = true;
						dodgedadachar = dad;
						dodgedadachar.playAnim('gun', true);
						dodgedadachar.specialAnim = true;
						health -= mechdamagegun;
						health -= bleed * .5;
						FlxG.camera.shake(0.03, .25);
				...
				new FlxTimer().start(0.5, function(timeThingh:FlxTimer)
				{
					serenityLightsPink.alpha = .5;
					FlxTween.tween(serenityLightsPink, {alpha: 0}, 1, {ease: FlxEase.cubeInOut});
					//image code here i guess
					//if you want, you can put "warning = false;" here instead of in the other timer
	...
	var stupidDUMBASS:Float = 0;
	var cameraSpinny:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		camHUD.angle += cameraSpinny * elapsed;
		...
		else if(songMisses == 10 || songMisses >= 10)
			{
				scoreFC = " // Clear";
			}
		...
		if(ratingString == "?") {
			scoreTxt.text = "Score: 0 // Misses: 0 // Accuracy: 0%" + scoreFC;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' // Misses: ' + songMisses + ' // Accuracy: ' + Math.floor(ratingPercent * 100) + '%' + scoreFC;
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				var strumX:Float = 0;
				var strumY:Float = 0 + stupidDUMBASS;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Add Camera Zoom':
				if(ClientPrefs.camZooms && FlxG.camera.zoom < 1.35 || goZoom) {
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					var difficulty:String = '-' + CoolUtil.difficultyStuff[storyDifficulty][0];
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image(pixelShitPart1 + daRating + pixelShitPart2 + fcExten));
		rating.screenCenter();
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				{
					case "furiousStage":
						padBF.playAnim(animToPlay, true);
						padBF.holdTimer = 0;
				}
	...
	var bangingGER:Bool = false;

	var sideToSide:Bool = false;
	var doSide:Bool;
	var funniDoSide:Float;
	var altSide:Bool = false;
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 4 == 0 && goZoom)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		...
		if(bangingGER)
			{
				if(altSide)
					...
					{
						FlxTween.tween(camGame, {angle: 0}, 0.7, {ease: FlxEase.quadInOut, startDelay: 0.5});
						camGame.angle = -10;
					}
			}
		else
		...
		funniDoSide = FlxG.random.float(-10, 15);

		if(doSide)
			{
				if(sideToSide)
					{
						camHUD.angle = -funniDoSide;
						camGame.angle = funniDoSide;
					}
				else
					{
						camHUD.angle = funniDoSide;
		...
		var idleWait:Int = SONG.bpm >= 250 ? 4 : (SONG.bpm >= 100 ? 2 : 1);

		/*
		
		var idleWait:Int;
		
		if(SONG.bpm >= 100 || SONG.bpm == 100)
			{
				idleWait = 2;
			}
		else if(SONG.bpm >= 250 || SONG.bpm == 250)
			{
				idleWait = 4;
			}
		else
		...
		switch (curStage)
		{
			...
			case 'furiousStage':
				if(curBeat % idleWait == 0)
					{
						if(padBF.animation.curAnim.name != null && !padBF.animation.curAnim.name.startsWith("sing"))
							{
								padBF.dance();
							}
					}
